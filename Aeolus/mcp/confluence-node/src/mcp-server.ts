...
const { CONF_BASE_URL, CONF_USERNAME, CONF_SPACE, CONF_TOKEN } = process.env;

// 支持多个默认 Space，逗号分隔，第一个为 create/update 的默认 Space
const confSpaceList: string[] = CONF_SPACE ? CONF_SPACE.split(",").map((s) => s.trim()).filter(Boolean) : [];
const defaultSpace: string = confSpaceList[0] ?? "";

// 创建 axios 认证配置
function getAxiosAuthConfig(): { headers: Record<string, string> } {
  return {
    headers: {
...
async function getPage(space: string, title: string): Promise<ConfluencePage | undefined> {
  // 当传入的 space 是默认的第一个 Space 时，依次尝试所有配置的 Space
  const spacesToTry = space === defaultSpace && confSpaceList.length > 1 ? confSpaceList : [space];
  for (const s of spacesToTry) {
    try {
      ...
      if (res.data.results[0]) return res.data.results[0];
    } catch {
      // 该 space 查找失败，继续尝试下一个
    }
  }
  return undefined;
}
...
async function searchPages(space: string | undefined, query: string, limit = 25): Promise<ConfluenceSearchResult[]> {
  try {
    let cql: string;
    if (space) {
      cql = `space=${space} AND title~"${query}"`;
    } else if (confSpaceList.length > 0) {
      const spaceFilter = confSpaceList.map((s) => `"${s}"`).join(", ");
      cql = `space IN (${spaceFilter}) AND title~"${query}"`;
    } else {
      cql = `title~"${query}"`;
    }
...
async function uploadAttachmentToPage({
  ...
  if (!CONF_BASE_URL) throw new Error("缺少环境变量 CONF_BASE_URL");
  if (!CONF_TOKEN) {
    throw new Error("缺少认证配置：请设置 CONF_TOKEN");
  }
  ...
  try {
    switch (name) {
      ...
      case "confluence_create_page": {
        const space = (args.space as string | undefined) || defaultSpace;
